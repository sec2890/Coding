...
<style type="text/css">
   body { background: #D2F7F2 !important; } /* Adding !important forces the browser to overwrite the default style applied by Bootstrap */
</style>
<title>Meet-Ups</title>
</head>
<body>
	<div id="container">
		<h1 class="">Welcome, <c:out value="${user.firstName}  ${user.lastName}"/></h1>
		<a class="fltright" href="/logout"><h2>Logout</h2></a>
		<h2>Health Meet-Ups in your state:</h2>
		<c:if test="${instate.size() == 0}"><h5>There are currently no Health related Meet-Ups in your area...</h5></c:if>
		...
		<h2>Health Meet-Ups in OTHER states:</h2>
		<table class="table table-striped table-bordered table-hover">
			<thead>
  				<tr>
    				<th scope="col">Name</th>
				...
				<c:forEach items="${outofstate}" var="out">
  				<tr>
				    ...
				    <c:choose>
                        ...
                        <c:otherwise>
                            <c:set var="attending" value="${false}"/>
                            <c:forEach items="${out.getJoinedUsers()}" var="attendee">
                                <c:if test="${attendee == user}">
                                    <c:set var="attending" value="${true}"/>
                                </c:if>
                            </c:forEach>
		...
		<div id="lftpnl">
			...
			<form:form method="post" action="/addevent" modelAttribute="eventObj">
				...
				<h4 class="avoid">
					...
					<form:select cssClass="show" path="state">
